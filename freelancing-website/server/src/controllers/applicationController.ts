...
export async function createApplication(req: AuthRequest, res: Response) {
  ...
  if (!proposal?.trim() || Number(bidAmount) <= 0) {
    return res.status(400).json({
      message: "A proposal and valid bid amount are required.",
    });
  }
...
export async function getMyApplications(req: AuthRequest, res: Response) {
  ...
  const applications = await Application.find({
    ...
    .populate(
      "jobId",
      "title company category budgetMin budgetMax duration workMode status"
    )
    .sort({ createdAt: -1 });
...
export async function getEmployerApplications(req: AuthRequest, res: Response) {
  if (!req.user || req.user.role !== "employer") {
    return res.status(403).json({
      message: "Only employers can access applicant data.",
    });
  }
  ...
  const applications = await Application.find({
    ...
    .populate(
      "jobId",
      "title company category budgetMin budgetMax duration workMode"
    )
    .populate(
      "freelancerId",
      "name email headline bio location skills languages rating completedProjects"
    )
    .sort({ createdAt: -1 });
...
export async function updateApplicationStatus(
  ...
  if (status === "accepted") {
    project = await Project.findOne({ jobId: job._id });
    ...
    await createNotification({
      userId: application.freelancerId.toString(),
...
export async function withdrawApplication(req: AuthRequest, res: Response) {
  ...
  if (application.status === "accepted") {
    return res.status(400).json({
      message: "An accepted application cannot be withdrawn.",
    });
  }
